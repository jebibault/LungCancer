...
layout = html.Div([
  html.P([html.Br()]),
  html.P([html.Br()]),
  dcc.Markdown('#### Seven questions to predict the risk of lung cancer death:'),
  ...
  html.P([html.Br()]),  
  html.P([html.Br()]),
  html.Div(html.P(['The predicted probability of lung cancer death is:']), style={'fontWeight': 'bold', 'color': '#48a1af', 'font-size': 'large', 'text-align': 'center'}),
  html.Div(id='LungCancerDeath-content', style={'fontWeight': 'bold', 'color': '#48a1af', 'font-size': 'large', 'text-align': 'center'}),
  html.Div(html.P(['This chart shows the most important features associated with a higher (red) or lower (blue) risk of lung cancer death']), style={'text-align': 'center'}),
  html.P([html.Br()])
])

@app.callback(
    Output('LungCancerDeath-content', 'children'),
    [Input('age', 'value'),
...
def predict(age,lung_cancer_family_history,smoking_status,age_started_smoking,years_smoking,packs,years_since_stopped_smoking):
  ...
  model = pickle.load(open('models/LungCancerDeath.pkl', 'rb'))
